...
class StateIndex 
{
	...
	public function getStates($request)
	{
        ...
        $order_by2 = $request->order_by2;
        $order_style = $request->order_style;
        $order_style2 = $request->order_style2;
        ...
        if (!$order_by) { $order_by = "country_id"; }
        if (!$order_style) { $order_style = "asc"; }

        if (!$order_by2) { $order_by2 = "name"; }
        if (!$order_style2) { $order_style2 = "asc"; }

        //arrange by columns
        $data = $data->orderBy($order_by, $order_style);
        $data = $data->orderBy($order_by2, $order_style2);
